...
- **最终方案：显式分行 + 按内容宽度分配列宽**（`packedRows`）。按 `displayWidth`（CJK/全角算 2，其余算 1）估算每个按钮宽度，用 `BROWSE_ROW_BUDGET = 32` 贪心装箱：短名字一行挤下 2–3 个，超预算的长名字**独占一行**（此时占满整卡宽度）。每行一个 `column_set`（`flex_mode: 'none'`，即上一版实测排版正常的写法），列宽用 `weight = 该标签显示宽度` 按比例分配，**条目名一律不省略**。控制按钮（⬆️/🏠/👁）与上一页/下一页同样装箱。
- **实机校验**：用 profile 凭据调飞书 `cardkit.v1.card.create` 验证生成的**工作区选择卡**与**目录浏览卡**，均 `code=0 success`；并检查生成的列结构——控制按钮 1 行 2 列、短目录 1 行 3 列、超长目录独占 1 行 1 列。
